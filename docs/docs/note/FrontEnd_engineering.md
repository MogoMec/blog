...
一个模块是实现一个特定功能的一组方法法的封装。

随着前端需求的复杂化，代码的模块化开发变得越来越重要。

最原始时简单地**使用函数**（利用函数的独立作用域）实现，几个函数为一个模块，但只要任意造成**全局变量污染**，且各个模块之前没有联系。

后来也有通过**对象**来实现模块，部分解决了全局变量污染，但是封装性不好，内部属性可以被外部代码修改。

常用的是**立即执行函数**实现，通过利用闭包来实现模块私有作用域的建立，同时不会对全局作用域造成污染。

现代模块化解决方案需要解决全局变量污染、数据保护的问题，同时还需要解决模块之间**依赖关系**的维护。
